...
// 🔹 Counter untuk nama tab yang sama
...
function addTab(title, component, icon = '📄') {
  ...
  // Untuk ReservationForm, buat counter untuk nama tab
  if (component === 'ReservationForm') {
    // Increment counter
    if (!componentCounters.value[component]) {
      componentCounters.value[component] = 0
    }
    componentCounters.value[component]++

    // Tambahkan nomor ke title jika lebih dari 1
    const count = componentCounters.value[component]
    if (count > 1) {
      title = `${title} ${count}`
    }
  } else if (component === 'WalkIn') {
    // Increment counter
    if (!componentCounters.value[component]) {
      componentCounters.value[component] = 0
    }
  ...
  const newTab = {
    ...
    icon,
    closeable: true,
    // 🔹 Tambahkan unique key untuk force re-render component baru
...
// 🔹 Close tab dengan update counter
function closeTab(tabId) {
  ...
  // Kurangi counter jika ReservationForm
  if (tab.component === 'ReservationForm') {
    if (componentCounters.value[tab.component] > 0) {
      componentCounters.value[tab.component]--;
    }
  } else if (tab.component === 'WalkIn') {
    if (componentCounters.value[tab.component] > 0) {
      componentCounters.value[tab.component]--;
    }
  }
...
<template>
  <div class="min-h-screen bg-gray-50">
      ...
      <div class="mx-auto max-w-full px-4">
        <div class="flex h-16 items-center justify-between">
          ...
          <div class="flex items-center gap-4">
            <button @click="showSidebar = !showSidebar" class="p-2 rounded-lg hover:bg-gray-100 transition">
          ...
          <div v-if="hasTabs" class="flex-1 mx-6 overflow-x-auto">
            <div class="flex items-center gap-1">
              <div v-for="tab in tabs" :key="tab.id" @click="switchTab(tab.id)"
                ...
                <span>{{ tab.title }}</span>
                <button @click.stop="closeTab(tab.id)" class="p-0.5 rounded transition-colors"
    ...
    <transition name="fade">
      <div v-if="showSidebar" @click="showSidebar = false" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-30">
      </div>
    </transition>

    <!-- Sidebar Menu -->
    <transition name="slide">
      <div v-if="showSidebar" class="fixed left-0 top-0 bottom-0 w-80 bg-white shadow-2xl z-40 overflow-y-auto">
        <!-- Sidebar Header -->
        <div class="p-4 border-b bg-gradient-to-r from-indigo-600 to-purple-600">
          <div class="flex items-center justify-between">
            ...
            <button @click="showSidebar = false" class="p-2 rounded-lg hover:bg-white/20 text-white transition">
        ...
        <!-- Menu Categories -->
        <div class="p-4">
          <div v-for="category in navigationMenus" :key="category.title" class="mb-6">
            <!-- Category Header -->
            ...
            <div class="space-y-1">
              <template v-for="item in category.items" :key="item.title">
                ...
                <details v-if="item.submenu" class="group">
                  <summary
                    class="flex items-center gap-3 px-4 py-2.5 rounded-lg hover:bg-indigo-50 cursor-pointer transition">
                  ...
                  <div class="ml-8 mt-1 space-y-1">
                    <button v-for="subitem in item.submenu" :key="subitem.title" @click="openMenuItem(subitem)"
                ...
                <!-- No Submenu -->
                <button v-else @click="openMenuItem(item)"
    ...
    <main class="min-h-[calc(100vh-4rem)]">
      ...
      <div v-if="hasTabs">
        <div v-for="tab in tabs" :key="tab.id" v-show="activeTabId === tab.id" class="animate-fadeIn">
          <!-- 🔹 Render komponen dinamis dengan unique key untuk force new instance -->
          <component :is="componentMap[tab.component]" :key="tab.componentKey" v-if="componentMap[tab.component]" />
          ...
          <div v-else class="p-6">
            <div class="mx-auto max-w-7xl">
              <div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
                <div
                  class="px-6 py-4 bg-gradient-to-r from-indigo-50 to-purple-50 border-b flex items-center justify-between">
                ...
                <div class="p-8">
                  <div class="text-center py-16">
                    ...
                    <div
                      class="max-w-2xl mx-auto bg-gradient-to-br from-gray-50 to-gray-100 p-12 rounded-xl border-2 border-gray-200">
